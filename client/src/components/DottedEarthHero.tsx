...
function ContinentalSurface({ longitude, patternId }: { longitude: number; patternId: string }) {
  ...
  return <g><path d={landPath} fill={`url(#${patternId})`} stroke="rgba(241,243,238,.22)" strokeWidth=".45" opacity=".9" /></g>;
}
  ...
  return <div aria-hidden className={`pointer-events-none absolute z-0 ${presentation.anchorClass} top-[5.5rem] sm:top-[7rem] ${presentation.sizeClass}`}>
    <svg viewBox="0 0 240 240" className={`h-full w-full overflow-visible ${presentation.routeClass}`} fill="none">
      <defs>
        ...
        <pattern id={`earth-points-${direction}-${density}`} width={density === "mobile" ? 5.6 : 4.2} height={density === "mobile" ? 5.6 : 4.2} patternUnits="userSpaceOnUse"><circle cx="1.6" cy="1.4" r={density === "mobile" ? ".75" : ".62"} fill="#f1f3ee" /><circle cx="4.5" cy="3.8" r={density === "mobile" ? ".36" : ".28"} fill="#f1f3ee" opacity=".66" /></pattern>
        <radialGradient id={`earth-base-${direction}-${density}`} cx="35%" cy="27%" r="74%"><stop stopColor="#545754" stopOpacity=".48" /><stop offset=".45" stopColor="#1c1d1d" stopOpacity=".95" /><stop offset="1" stopColor="#080909" /></radialGradient>
        <radialGradient id={`earth-light-${direction}-${density}`} cx="38%" cy="30%" r="50%"><stop stopColor="#f1f3ee" stopOpacity=".36" /><stop offset=".5" stopColor="#dfe3dd" stopOpacity=".08" /><stop offset="1" stopColor="#f1f3ee" stopOpacity="0" /></radialGradient>
      </defs>
      <ellipse cx="120" cy="120" rx="113" ry="99" stroke="rgba(241,243,238,.18)" strokeWidth=".55" />
      <ellipse cx="120" cy="120" rx="106" ry="92" stroke="rgba(241,243,238,.10)" strokeWidth=".45" strokeDasharray="2 4" />
      <g clipPath={`url(#earth-mask-${direction}-${density})`}>
        ...
        <g opacity=".12"><ellipse cx="120" cy="120" rx="58" ry="92" stroke="#f1f3ee" strokeWidth=".45" /><ellipse cx="120" cy="120" rx="78" ry="92" stroke="#f1f3ee" strokeWidth=".35" /><path d="M30 84c51 18 99 18 180 0M28 121c54 12 107 12 184 0M34 158c47-17 101-17 172 0" stroke="#f1f3ee" strokeWidth=".35" /></g>
        <circle cx="120" cy="120" r="92" fill={`url(#earth-light-${direction}-${density})`} />
      </g>
      <circle cx="120" cy="120" r="92" stroke="rgba(241,243,238,.36)" strokeWidth=".65" />
      <g opacity=".42"><path d="M-5 157C47 115 91 91 140 97c41 5 67 29 105 8" stroke="rgba(241,243,238,.46)" strokeWidth=".55" /><path d="M0 55c62 27 105 20 144 2 43-20 66-12 99 23" stroke="rgba(241,243,238,.30)" strokeWidth=".45" strokeDasharray="1.5 4" /></g>
      {allowMotion && <><circle className="nf-earth-route-dot" cx="148" cy="103" r="1.35" fill="#f1f3ee" /><circle className="nf-earth-route-dot" cx="126" cy="67" r="1.05" fill="#f1f3ee" /></>}
    </svg>
  </div>;
}
