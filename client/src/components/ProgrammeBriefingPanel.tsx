...
export function ProgrammeBriefingPanel({ briefing, isGenerating, isArabic, onGenerate }: { briefing?: ProgrammeBriefingRecord; isGenerating: boolean; isArabic: boolean; onGenerate: () => void }) {
  ...
  return <section className="mt-3 border border-white/15 bg-black/20 p-3">
    <div className="flex flex-wrap items-start justify-between gap-3"><div><p className="nf-label text-[8px] text-[#aab4ae]">// {copy.kicker}</p>{briefing && <p className="mt-1 text-[9px] text-[#8e9992]">{copy.updated}: {generatedLabel}</p>}</div><button type="button" disabled={isGenerating} onClick={onGenerate} className="nf-button inline-flex items-center gap-2 border border-white/25 px-2.5 py-2 text-[9px] font-semibold hover:bg-white hover:text-black disabled:opacity-50"><Sparkles className="h-3 w-3" />{briefing ? copy.refresh : copy.generate}</button></div>
    <AnimatePresence mode="wait">{isGenerating ? <motion.div key="loading" initial={reduceMotion ? false : { opacity: 0, y: 5 }} animate={{ opacity: 1, y: 0 }} exit={{ opacity: 0, y: -5 }} transition={{ duration: .18 }} className="mt-3 flex items-center gap-2 border-t border-white/10 pt-3 text-xs text-[#dce4df]"><Loader2 className="h-3.5 w-3.5 animate-spin" />{copy.loading}</motion.div> : showBriefing ? <motion.div key="briefing" initial={reduceMotion ? false : { opacity: 0, y: 5 }} animate={{ opacity: 1, y: 0 }} exit={{ opacity: 0, y: -5 }} transition={{ duration: .2 }} className="mt-3 border-t border-white/10 pt-3"><div><p className="nf-label text-[8px] text-[#9da79f]">{copy.signals}</p><ul className="mt-2 space-y-1.5">{briefing.keyFitSignals.map((signal) => <li key={signal} className="flex gap-2 text-[11px] leading-5 text-[#e0e6e2]"><span aria-hidden="true">—</span><span>{signal}</span></li>)}</ul></div><div className="mt-3 grid gap-3 sm:grid-cols-2"><BriefFact label={copy.language} value={briefing.languageRequirements} /><BriefFact label={copy.cost} value={briefing.costContext} /><BriefFact label={copy.admission} value={briefing.admissionContext} /><BriefFact label={copy.next} value={briefing.nextResearchStep} /></div><div className="mt-3 flex flex-wrap items-center gap-3 border-t border-white/10 pt-3"><a href={briefing.sourceUrl} target="_blank" rel="noreferrer" className="nf-button inline-flex items-center gap-1 text-[10px] font-semibold underline hover:text-white">{copy.source}<ArrowUpRight className="h-3 w-3" /></a><span className="flex items-center gap-1 text-[10px] leading-4 text-[#b9c3bc]"><ShieldCheck className="h-3.5 w-3.5 shrink-0" />{briefing.reviewNote || copy.boundary}</span></div></motion.div> : <motion.p key="empty" initial={false} className="mt-3 border-t border-white/10 pt-3 text-[11px] leading-5 text-[#9fa9a2]">{copy.empty}</motion.p>}</AnimatePresence>
  </section>;
}

function BriefFact({ label, value }: { label: string; value: string }) {
  return <div className="border-l border-white/15 pl-3"><p className="nf-label text-[8px] text-[#9da79f]">{label}</p><p className="mt-1 text-[11px] leading-5 text-[#dce4df]">{value}</p></div>;
}
