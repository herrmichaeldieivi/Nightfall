...
describe("guided student journey state", () => {
  ...
  it("treats discovery as a one-time review action after matching context exists", () => {
    ...
    expect(state.enabledTools).toEqual(["consult", "discover"]);
  });
  ...
  it("keeps the Consultant reachable at every later stage as long as re-interview budget remains, and drops it only when the budget is spent", () => {
    ...
    expect(laterStageNoBudget.enabledTools).not.toContain("consult");
    expect(resolveJourneyDestination({ ...base, hasMatchingContext: true, savedProgrammeCount: 2, remainingConsultations: 0 }, "consult").destination).toBe("home");
  });
