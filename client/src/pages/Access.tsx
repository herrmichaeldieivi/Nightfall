...
// Second-step labels for the #163 email-code gate, kept separate so the
// long single-line copy block above stays untouched.
const codeCopy = { en: { codeTitle: "Check your inbox.", codeBody: "We sent a six-digit code to your email. It expires in ten minutes.", code: "Six-digit code", verifyCreate: "Verify and create account", changeEmail: "Use a different email" }, ar: { codeTitle: "تفقد بريدك.", codeBody: "بعتنا كود من ست أرقام على إيميلك. بينتهي بعد عشر دقايق.", code: "كود من ستة أرقام", verifyCreate: "تحقق وأنشئ الحساب", changeEmail: "استخدم إيميل ثاني" } } as const;
...
function AccessPage({ mode, onBack, onDone, onOther }: { mode: "login" | "signup"; onBack: () => void; onDone: (profile?: { onboardingComplete?: boolean }) => void; onOther: () => void }) {
  ...
  const submit = async (event: FormEvent) => {
    ...
    try {
      if (signUp && !codeStep) {
        // Step one of the #163 gate: prove ownership of the address before any
        // account exists. The server rate-limits and stores only hashed codes.
      ...
      const response = await fetch(signUp ? "/api/auth/register" : "/api/auth/login", { method: "POST", headers: { "Content-Type": "application/json" }, credentials: "include", body: JSON.stringify(signUp ? { name, email, password, unlockToken } : { email, password }) });
  ...
  return <div dir={isArabic ? "rtl" : "ltr"} className="night-bloom grid min-h-screen lg:grid-cols-[1.05fr_.95fr]">
    ...
    <section className="grid place-items-center p-5 sm:p-10"><div className="w-full max-w-md"><div className="flex items-center justify-between"><button onClick={onBack} className="nf-label flex items-center gap-2 text-[#9ca79f] lg:hidden"><ArrowLeft className={`h-3.5 w-3.5 ${isArabic ? "rotate-180" : ""}`} />{t.back}</button><LanguageToggle language={language} onToggle={toggleLanguage} /></div><div className="mt-10 lg:mt-0"><div className="flex items-center justify-between"><Mark /><span className="nf-mono text-[10px] text-[#8c9790]">{signUp ? "ACCESS / CREATE" : "ACCESS / SIGN IN"}</span></div><p className="nf-label mt-10 text-[#9fa9a3]">// {signUp ? t.labelSignup : t.labelLogin}</p><h2 className="mt-3 text-4xl font-semibold leading-[.95] tracking-[-.05em] text-white">{signUp ? t.titleSignup : t.titleLogin}</h2><p className="mt-4 text-sm leading-6 text-[#a0aba4]">{signUp ? t.bodySignup : t.bodyLogin}</p>
      <form onSubmit={submit} className="mt-8 space-y-3">
        {signUp && codeStep ? (
          <>
            <p className="flex items-start gap-2 text-sm leading-6 text-[#d9dfda]"><MailCheck className="mt-0.5 h-4 w-4 shrink-0" /><span><span className="block font-semibold text-white">{t.codeTitle}</span>{t.codeBody}</span></p>
            <label className="block border border-white/15 bg-black/20 px-4 py-3"><span className="nf-label block text-[8px] text-[#8e9992]">{t.code}</span><input inputMode="numeric" autoComplete="one-time-code" pattern="\d*" maxLength={6} required value={code} onChange={(event) => setCode(event.target.value.replace(/\D/g, ""))} className="mt-1 w-full bg-transparent text-lg tracking-[.4em] text-white outline-none placeholder:text-white/30" /></label>
            <button type="button" onClick={() => { setCodeStep(false); setCode(""); setError(null); }} className="text-xs text-[#9ca79f] underline underline-offset-4 hover:text-white">{t.changeEmail}</button>
          </>
        ) : (
          <>
        ...
        <button type="submit" disabled={busy} className="nf-button flex w-full items-center justify-center gap-2 border border-white bg-white px-4 py-3.5 text-[10px] font-bold uppercase tracking-[.08em] text-black disabled:opacity-60">{busy ? <Loader2 className="h-4 w-4 animate-spin" /> : null}{signUp ? (codeStep ? t.verifyCreate : t.actionSignup) : t.actionLogin}{!busy && <ArrowRight className={`h-4 w-4 ${isArabic ? "rotate-180" : ""}`} />}</button>
      </form>
