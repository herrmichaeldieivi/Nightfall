...
function DocumentBody({ doc }: { doc: Doc }) {
  return <div className="space-y-6">{doc.sections.map((section) => <section key={section.heading}><h3 className="text-sm font-semibold text-white">{section.heading}</h3><p className="mt-2 text-sm leading-6 text-[#b9c1bc]">{section.body}</p></section>)}</div>;
}
...
export function AcceptGate({ children }: { children: React.ReactNode }) {
  ...
  return <div dir={isArabic ? "rtl" : "ltr"} className="night-bloom grid min-h-screen place-items-center p-5 text-white"><div className="w-full max-w-2xl border border-white/15 bg-[#111516] p-6 shadow-[0_24px_80px_rgba(0,0,0,.45)] sm:p-8"><div className="flex items-center justify-between"><p className="nf-label text-[#9fa9a3]">// BEFORE YOU CONTINUE / V{LEGAL_VERSION}</p><LanguageToggle language={language} onChange={setLanguage} /></div>
    <div className="mt-5 flex flex-wrap gap-x-5 gap-y-2 border-b border-white/10 pb-3">{(Object.keys(docs) as DocKey[]).map((key) => <button key={key} onClick={() => setTab(key)} className={`text-xs font-semibold ${tab === key ? "text-white underline underline-offset-8" : "text-[#929d96] hover:text-white"}`}>{titles[key]}</button>)}</div>
