...
export type ApplicationEventType = "programme_saved" | "programme_archived" | "programme_priority_set" | "programme_priority_cleared" | "decision_notes_updated" | "consultation_completed" | "document_uploaded" | "document_verified" | "deadline_confirmed" | "communication_drafted" | "communication_approved" | "communication_sent" | "communication_reply_received" | "follow_up_planned" | "follow_up_completed" | "application_submitted" | "admission_offer_received" | "application_rejected";
...
export async function searchGermanyProgrammeIndex(input: { query?: string; category?: string; language?: string; limit: number }) {
  ...
  const conditions = [];
  if (query) conditions.push(or(like(germanyProgrammeIndex.programmeName, `%${query}%`), like(germanyProgrammeIndex.officialName, `%${query}%`), like(germanyProgrammeIndex.city, `%${query}%`)));
...
export async function listGermanyProgrammeCandidatesForFit(input: { studyDirection: string; limit?: number }) {
  ...
  const conditions = direction ? [or(like(germanyProgrammeIndex.broadSubjectCategories, `%${direction}%`), like(germanyProgrammeIndex.programmeName, `%${direction}%`), like(germanyProgrammeIndex.fieldMatchBasis, `%${direction}%`))] : [];
