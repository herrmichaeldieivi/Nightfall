...
export async function syncUniversityRepliesFromInbox(userId: UserId, language: "en" | "ar") {
  ...
  for (const reply of replies.slice(0, 10)) {
    ...
    if (classification.success) await categorizeUniversityInboundCommunication(userId, { communicationId: result.communicationId, category: classification.data.category, nextStep: classification.data.nextStep, reviewNote: classification.data.reviewNote });
  }
  return { imported };
}
