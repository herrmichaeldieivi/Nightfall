...
const terms = (value?: string | null) => (value ?? "").toLowerCase().split(/[,&/;|]+/).map((item) => item.trim()).filter((item) => item.length > 1);
...
export function explainProgrammeMatch(profile: FitProfileForMatching, programme: ProgrammeResearchRecord): ProgrammeMatch {
  ...
  if (profile.tuitionBudgetBand) { fitSignals.push("Your budget preference is retained for review; current programme cost must be verified from official evidence."); score += 4; }
  if (profile.hasSponsor || profile.fundingRoute) fitSignals.push("Your funding route is retained for planning; visa and financial-proof rules require official confirmation.");
