...
export async function primeUniversityRequirementWatch(input: { userId: number; watch: { id: number; universityId: number; sourceUrl: string; sourceLabel: string } }) {
  const response = await fetch(input.watch.sourceUrl, { headers: { "User-Agent": "Nightfall requirements watch/1.0 (+student review)" }, signal: AbortSignal.timeout(25_000) });
...
export async function runUniversityRequirementsWatchForSchedule(scheduleCronTaskUid: string) {
  ...
  const watches = (await listUniversityRequirementWatches(preferences.userId)).filter((watch) => watch.enabled);
  let checked = 0;
  let changed = 0;
  ...
  for (const watch of watches) {
    try {
      const prime = await primeUniversityRequirementWatch({ userId: preferences.userId, watch });
      checked += 1;
