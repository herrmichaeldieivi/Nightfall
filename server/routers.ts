...
export const studentProfileInput = z.object({ preferredName: z.string().trim().min(2).max(120), contactEmail: z.string().trim().email().max(320), phoneNumber: z.string().trim().min(7).max(80), destination: z.string().trim().min(2).max(200), graduationYear: z.string().trim().min(2).max(32), highSchoolDiplomaOrigin: z.string().trim().min(2).max(200), preferredLanguage: z.enum(["en", "ar"]) });
...
const consultingInput = z.object({ language: z.enum(["en", "ar"]), messages: z.array(consultingMessageSchema).min(1).max(10) });
...
export const appRouter = router({
  ...
  student: router({
    ...
    updateReminderPreferences: protectedProcedure.input(reminderPreferenceInput).mutation(({ ctx, input }) => {
      const sessionToken = parseCookie(ctx.req.headers.cookie ?? "")[COOKIE_NAME] ?? "";
      return updateReminderPreferencesWithSchedule(ctx.user.id, input, sessionToken);
    }),
    ...
    updateUniversityWatchPreferences: protectedProcedure.input(universityWatchPreferenceInput).mutation(({ ctx, input }) => {
      const sessionToken = parseCookie(ctx.req.headers.cookie ?? "")[COOKIE_NAME] ?? "";
      return updateUniversityWatchPreferencesWithSchedule(ctx.user.id, input, sessionToken);
    }),
